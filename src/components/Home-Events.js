...
class HomeEvents extends Component {
  ...
  createEvents = () => {
    if (!this.props.userData || !this.props.userData.events
      || this.props.userData.events.length === 0) {
      return (
        <div className="Events col">
          <div className="Events-contr">
            ...
            <div className="Events-contr-date" />
          </div>
        </div>
      );
    }
    const { events } = this.props.userData;
    return (
      <div className="Events col">
        {
        ...
        <div className="Events-contr" key="-1">
          ...
          <div className="Events-contr-summary">
              View all events,
            {' '}
            <b><a href={ this.userUrl() } target="_blank" rel="noopener noreferrer">{this.userUrl()}</a></b>
          </div>
        </div>
      </div>
    );
  ...
  getEventIcon = (type, action, ref_type, merged) => {
    switch (type) {
    case 'IssuesEvent':
      if (action === 'closed') return (<GoIssueClosed />);
      if (action === 'reopened') return (<GoIssueReopened />);
      return (<GoIssueOpened />);
    case 'CreateEvent':
      if (ref_type === 'repository') return (<GoRepo />);
      if (ref_type === 'branch') return (<GoGitBranch />);
      if (ref_type === 'tag') return (<FaTag />);
      return (<FaPencilAlt />);
    case 'PullRequestEvent':
      if (merged) return (<GoGitMerge />);
      return (<GoGitPullRequest />);
    case 'PullRequestReviewEvent':
      if (action === 'submitted') return (<FaCheck />);
      if (action === 'edited') return (<FaPencilAlt />);
      if (action === 'dismissed') return (<FaTimes />);
      return (<FaEye />);
    case 'PushEvent':
      return (<GoGitBranch />);
    case 'WatchEvent':
  ...
  getEventDesc = (type, action, ref_type, merged) => {
    ...
    case 'PullRequestReviewEvent':
      ...
      break;
    case 'CreateEvent':
      if (ref_type === 'repository') text = 'Created a repo';
      else if (ref_type === 'branch') text = 'Created a branch';
      else if (ref_type === 'tag') text = 'Created a tag';
      else text = 'Created something';
      break;
    case 'PullRequestEvent':
      if (action === 'closed' && merged) text = 'Merged a pull request';
      else if (action === 'closed' && !merged) text = 'Closed a pull request';
      else if (action === 'review_requested') text = 'Requested a pull request review';
      else if (action === 'review_requested_removed') text = 'Removed a pull request review';
      else text = `${action} a pull request`;
      break;
    case 'PushEvent':
  ...
  getEventRepoUrl = url => url.replace(/(api\.|\/repos)/g, '')
